...
const Home = () => {
	...
	useEffect(() => {
		axios
			.get(
				"https://api.open-meteo.com/v1/forecast?latitude=52.23&longitude=21.01&hourly=temperature_2m,weathercode,apparent_temperature,winddirection_10m,snowfall,relativehumidity_2m,windspeed_10m,cloudcover,rain,surface_pressure&daily=sunrise,sunset,shortwave_radiation_sum,uv_index_max&timezone=Europe%2FLondon"
			)
			.then((res) => {
				const data = res.data;
	...
	return (
		<>
			...
			<div className="flex lg:flex-row flex-col lg:mx-40 lg:mt-5 text-lg ">
				...
				{data
					.filter((item) => item.date === tomorrowString && item.hours === "12")
					.map((item) => (
